...
<template>
  <div id="msg">
    <div class="title">
<!--      <span>试卷列表</span>
      <span>/  {{workData.source}}</span>-->
    </div>
    <div class="wrapper">
      <ul class="top">
        <li class="example">{{workData.source}}</li>
<!--        <li><i class="iconfont icon-pen-"></i></li>
      ...
      <ul class="bottom">
        <li>更新于{{workData.examDate}}</li>
        <li>来自 {{workData.institute}}</li>
        <li class="btn">{{workData.type}}</li>
        <li class="right"><el-button @click="toAnswer(workData.workCode)">开始答题</el-button></li>
      </ul>
      <ul class="info">
        <li @click="dialogVisible = true"><a href="javascript:;"><i class="iconfont icon-info"></i>答题须知</a></li>
      </ul>
    </div>
    <div class="content">
      <el-collapse v-model="activeName" >
        <el-collapse-item class="header" name="0">
          <template slot="title" class="stitle" >
            <div class="title">
              <span>{{workData.source}}</span><i class="header-icon el-icon-info"></i>
<!--              <span class="time">{{workData.totalScore}}分 / {{workData.totalTime}}分钟</span>-->
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      this.$axios(`/api/work/${workCode}`).then(res => {  //通过examCode请求试卷详细信息
        res.data.data.createTime = res.data.data.createTime.substr(0,10)
    ...
    toAnswer(id) {
      this.$router.push({path:"/work/answer",query:{workCode: id}})
    },
  }
}
</script>
